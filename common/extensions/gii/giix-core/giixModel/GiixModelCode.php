...
Yii::import('common.extensions.gii.giix-core.helpers.*');
...
class GiixModelCode extends ModelCode {
		...
		foreach ($tables as $table) {
			...
			if (isset($this->relations[$className])) {
				foreach ($this->relations[$className] as $relationName => $relationData) {
					if (preg_match('/^\[self::MANY_MANY,.*?,\s*\'(.+?)\[/', $relationData, $matches)) {
	...
	public function generateRules($table) {
		...
		if ($null !== array())
            $rules2[] = "['" . implode(', ', $null) . "', 'default', 'setOnEmpty' => true, 'value' => null]";
	...
	public function getRelationData($className, $relationName, $relations = array()) {
		...
		$relationData = array();
		if (preg_match("/^\[([\w:]+?),\s?'(\w+)',\s?'([\w\s\(\),]+?)'\]$/", $relation, $matches_base)) {
