...
class TennisAction extends CAction
{
    public function run()
    {
        ...
        $Events = \TennisEvent::model()->findAll($criteria);

        $criteria = new \CDbCriteria();
        $criteria->addInCondition('type', ['ratio_p1', 'ratio_p2']);
        ...
        $this->getController()->setPageTitle('Главная - Теннис');
        Yii::app()->getAjax()
            ->addTrigger('page:loaded')
            ->addHtml('main/tennis', '#content-replacement #tab-content', $params)
            ->send();
    }
}