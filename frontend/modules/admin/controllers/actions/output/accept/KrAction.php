...
class KrAction extends CAction
{
    public function run()
    {
        ...
        try {
            ...
            $criteria->addCondition('`t`.price_type = :kr');
            $criteria->with = ['user'];
            $criteria->params = [
            ...
            $Moder = TransferFactory::factory('moder', $Request->getPriceType(), [$User]);
            if (!$Moder->accept($Request->getBalanceId()))
                throw new NException("Не удалось вывести кр", NException::ERROR_FINANCE_OUT);
