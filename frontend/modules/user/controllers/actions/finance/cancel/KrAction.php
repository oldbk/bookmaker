...
class KrAction extends CAction
{
    ...
    public function run()
    {
        ...
        try {
            ...
            if (!$Request) {
                Yii::app()->getAjax()->addErrors('Запрос не найден');
                throw new \Exception();
            }
            ...
            if (!$Moder->cancel($Request->getBalanceId()))
                throw new NException("Не удалось отменить запрос на вывод", NException::ERROR_FINANCE_IN);
            ...
            Yii::app()->getAjax()
                ...
                ->addMessage('Запрос на вывод отменен');
        } catch (\Exception $ex) {
            $t->rollback();
