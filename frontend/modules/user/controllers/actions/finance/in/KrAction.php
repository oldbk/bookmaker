    ...
    public function run()
    {
        ...
        try {
            ...
            if (Yii::app()->getUser()->getLevel() < $min_level) {
                Yii::app()->getAjax()->addErrors("Ввод eкр доступен с {$min_level} уровня");
                throw new \Exception();
            }

            if (Yii::app()->getUser()->getAlign() == 4) {
                Yii::app()->getAjax()->addErrors("Невозможен ввод средств: склонность хаос.");
                throw new \Exception();
            }
            ...
            $form = Yii::app()->getRequest()->getPost('krIn', []);
            ...
            if (!Yii::app()->getOldbk()->canTakeKr($User->getGameId(), $price)) {
                Yii::app()->getAjax()->addErrors('Недостаточно средств');
                throw new \Exception();
            }

            $Input = TransferFactory::factory('io', iPrice::TYPE_KR, [$User]);
            if (!$Input->add($price)) {
                Yii::app()->getAjax()->addErrors('Не удалось пополнить кр');
                throw new \Exception();
            }
            ...
            Yii::app()->getAjax()
                ->addMessage('Вы успешно пополнили кр')
                ->runJS('closeModal')
